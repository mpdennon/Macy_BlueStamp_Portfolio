...
Line tracking and distance detection come as first nature to people, but technology does it better, as with most things. The goal: have a robot that moves along a line and then follows an object to hop back on a different set of tracks. I am happy to report I did just that. 
...
For this milestone, I tackled a lot. I first tested the ultrasonic sensor by running a distance test then from there, I figured out how to work the servo motor to swivel the sensor. The sensor told me distance readings every ten degrees in cm using an array. I then ensured it could analyze the shortest output, then turned the servo motor to it. From there, I added the motors in to have the whole car turn to the shortest angle. This posed a unique challenge because initially, the car was turning in the right direction but not stopping at the correct angle. This is when I learned about maps and how to use them to do just that
